...
<div class="container">
    ...
    <div class="invoice-meta">
        <div class="customer-info">
            ...
            @if($invoice->customer->country && $invoice->customer->country !== 'Deutschland'){{ $invoice->customer->country }}@endif
        </div>

        <div class="invoice-details">
            <div class="section-title">Rechnungsdetails</div>
            <div class="info-row">
                <span class="info-label">Rechnungsnr.:</span>
            ...
            @if($layout->settings['content']['show_customer_number'] ?? true && $invoice->customer->customer_number)
                <div class="info-row">
    ...
    <div class="totals">
        <table class="totals-table">
            ...
            <tr>
                <td><strong>MwSt. ({{ number_format($invoice->tax_rate * 100, 0) }}%):</strong></td>
    ...
    <!-- Notes -->
    @if($invoice->notes && ($layout->settings['content']['show_notes'] ?? true))
        <div class="notes">
    ...
    @if($layout->settings['branding']['show_footer'] ?? true)
        <div class="footer">
            ...
            @if($layout->settings['content']['show_payment_terms'] ?? true)
                <p><strong>Zahlungsbedingungen:</strong> Zahlbar innerhalb von {{ \Carbon\Carbon::parse($invoice->due_date)->diffInDays(\Carbon\Carbon::parse($invoice->issue_date)) }} Tagen ohne Abzug.</p>
            @endif
        </div>
    @endif
</div>
